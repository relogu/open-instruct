...
def _resolve_vllm_parser_import_path(*candidates: tuple[str, str]) -> str:
    """Return the first importable vLLM parser path from a candidate list."""
    ...
    module_name, class_name = candidates[0]
    return f"{module_name}:{class_name}"
...
VLLM_PARSERS: dict[str, VllmParserConfig] = {
    ...
    "vllm_hermes": VllmParserConfig(
        import_path=_resolve_vllm_parser_import_path(
            ("vllm.tool_parsers.hermes_tool_parser", "Hermes2ProToolParser"),
    ...
    "vllm_llama3_json": VllmParserConfig(
        import_path=_resolve_vllm_parser_import_path(
            ("vllm.tool_parsers.llama_tool_parser", "Llama3JsonToolParser"),
            ("vllm.entrypoints.openai.tool_parsers.llama_tool_parser", "Llama3JsonToolParser"),
        ),
        role_templates={
    ...
    "vllm_olmo3": VllmParserConfig(
        import_path=_resolve_vllm_parser_import_path(
            ("vllm.tool_parsers.olmo3_tool_parser", "Olmo3PythonicToolParser"),
            ("vllm.entrypoints.openai.tool_parsers.olmo3_tool_parser", "Olmo3PythonicToolParser"),
            ("vllm.entrypoints.openai.tool_parsers.pythonic_tool_parser", "PythonicToolParser"),
        ),
        role_templates={
